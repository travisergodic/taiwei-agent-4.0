...
你的任务是判断当前搜集信息是否充分解决用户提问中的所有问题。若存在未解答的问题，生成一个新的提问句，涵盖所有用户提问未解决的问题。请遵循以下规则：
1. 依据当前信息与提问进行判断
   + 只基于当前提供信息和用户提问判断问题是否已解决，避免主观推测或分析。
2. 判断未解决的问题
   + 若用户提问中的部分或全部问题未得到明确解答，视为未解决。
   + 若信息对问题有所回应，但回答不明确或表明暂时无法回答，也视为未解决。
3. 生成简洁的后续提问
   + 新提问句必须简明扼要，包含所有未解决的子问题，并确保新提问句是完整且可解的。
   + 确保新提问使用明确的时间或地点，如 9月11号、周二、上海、北京，不要使用含糊不清的字眼，如今天、后天、那里。
   + 新提问句不得长于原用户提问句的长度。
4. 不得生成原问题之外的新问题。
...
2. 事实为基础：严格基于提供的当前信息进行回答，杜绝主观判断或评论。
...
# 1. 简短回复：根据当前信息，不要加入完全不相关的内容。
# 2. 事实为基础：根据提供的当前信息进行回答，不得加入主观判断或猜测。
# 3. 回复必须包含问题的关键内容，确保即使单独阅读回复，也能清楚了解问题和答案的含义
...
7. 特殊案例：
   + 若要比较多家餐馆或酒店价格（例如选出最便宜的烧烤店），请依据餐馆或酒店的起跳价格作为比较价格的依据。
...
我调用了该函数，并传入 {params} 作为参数，最终，函数返回：
{response}

请确认返回结果能够解决用户问题的部分，并返回对该部分问题有帮助的信息，并遵循以下原则：
1. 简洁回答：仅提供对问题有帮助信息。
2. 基于事实：仅依据返回结果提供回复，避免任何主观判断或推测。

返回结果需为 JSON 格式，包含以下字段：
+ `brief_response`：对用户问题的简要回复，类型为 string。
"""