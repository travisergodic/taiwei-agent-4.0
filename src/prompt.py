...
2. 判断未解决的问题
   ...
   + 若信息对问题有所回应，但回答不明确或表明暂时无法回答、无法确定，也视为未解决。
3. 生成简洁的后续提问
...
# 请根据当前信息判断用户问题中的所有问题是否已被解决。若存在未解决的问题，请生成一个新的提问句，涵盖所有用户问题中未解决的问题，并遵循以下规则：
# 1. 依据当前信息与提问进行判断
#    + 只基于当前提供信息和用户提问判断问题是否已解决，避免主观推测或分析。
# 2. 判断未解决的问题
#    + 若用户问题中部分或全部问题未得到明确解答，视为未解决。
#    + 若当前信息对部分问题有回应，但回答不明确或表明暂时无法回答、无法确定，也视为未解决。
# 3. 生成简洁的后续提问
#    + 新提问句必须简明扼要，包含所有未解决的子问题，并确保新提问句是完整且可解的。
#    + 确保新提问使用明确的时间或地点，如 9月11号、周二、上海、北京，不要使用含糊不清的字眼，如今天、后天、那里。
#    + 新提问句不得长于原用户提问句的长度。
...
2. 最多返回 {topk} 个适合解决用户问题的工具名称，并返回不少於 1 个工具。
3. 确保推荐的所有工具能解决用户提出的所有问题。
4. 不要有其他解释，或其他不相关的描述。

请以 list of string 格式回传工具名称，例如：[工具名称一, 工具名称二, ...]
"""
...
3. 回复顺序：回复问题的顺序与结构请与提问句保持一致。
4. 选择最能解决问题的信息：如果有多则信息与问题中的某个子问题相关，选择最能解决该子问题的信息来回复，切勿对同个子问题有两种不同的说法。
...
- 2024 年 10 月 1 日上海气平均温约 25 度

#### 回复：
2024 年 10 月 1 日上海平均气温约 25 度，从上海开车去扬州最短 2 小时
"""
...
请帮我根据函数返回的信息**对用户问题进行回答**，并返回**回答所依据的函数返回结果编号**，回复时必须遵守以下规则：
1. 简短回复：根据当前信息，不要加入不相关的内容。
2. 事实为基础：根据提供的当前信息进行回答，不得加入主观判断或猜测。
3. 精准匹配：从当前信息中筛选与问题最相关的事实。如果有多个事实与问题相关，选择最能解决问题且最可信的信息来回复。
4. 简单算术：若需要通过算术操作得出答案，请执行必要的计算。
5. 处理不足信息：如果某部分问题无法直接回答，但有与该部分问题相关信息，请简短提供这些信息；若完全没有相关信息，请明确表示 "无法获取相关信息"。
6. 完整句子：回答时必须包含问题的关键信息，以确保即使单独阅读回答，也能清楚理解问题和答案的含义。**若用户问题有提及地点、时间等信息，请在回复时也一併提及该信息。**
7. 特殊案例处理：
   + 若要比较多家餐馆或酒店价格（例如选出最便宜的餐馆），请依据餐馆或酒店的起跳价格作为比较价格依据。
   + 用户提问关于特定日期温度或两地温差，若仅有该日期所在时间段温度信息，请以该时间段温度信息作为该日期的温度信息。
   + 用户提问关于特定日期机票或车票价格，若仅能取得其他日期票价，回复时，请提供其他日期票价，并额外註明该票价日期。
   + 用户提问关于两地点最低票价、最短耗时，若调用工具后无法取得相关信息，也请将该函数返回结果编号加入回答所依据的编号中。
